...
// types from schema `public`. Note that this is
// also copied into each of the table packages.
//
// This should work with database/sql.DB and database/sql.Tx.

func Provider(in di.Injector) {
	di.Provide[DB](in, provide)
}

func provide(in di.Injector) (DB, error) {
	databaseURL, err := di.Load[db.URL](in)
	if err != nil {
		return nil, err
	}
	cfg, err := pgx.ParseConnectionString(string(databaseURL))
	if err != nil {
		return nil, err
	}
